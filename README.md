...
maildir.ai connects to large language models via an email interface.  Specifically, maildir_ai
watches a maildir directory for new messages, and then sends them to a language model for
processing.  The language model then generates a response, which maildir_ai "sends" back to the
sender.
